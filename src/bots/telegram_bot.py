...
# Gates finais — Tolerância e compensação
ATLAS_EDGE_TOLERANCE = float(os.getenv('ATLAS_EDGE_TOLERANCE', '0.005'))
ATLAS_PROB_COMPENSATION = float(os.getenv('ATLAS_PROB_COMPENSATION', '5.0'))
ATLAS_EDGE_COMPENSATION_FACTOR = float(os.getenv('ATLAS_EDGE_COMPENSATION_FACTOR', '0.50'))
ATLAS_MIN_SETUP_SCORE = int(os.getenv('ATLAS_MIN_SETUP_SCORE', '4'))

# Anti-spam por ativo
MONITOR_MAX_SIGNALS_PER_SYMBOL_HOUR = int(os.getenv('MONITOR_MAX_SIGNALS_PER_SYMBOL_HOUR', '2'))
...
def _prune_symbol_signal_history(symbol: str, now: datetime):
    """Remove entradas de sinal antigo (>1 hora) para evitar vazamento de memória."""
    try:
...
def _can_send_symbol_signal(symbol: str, now: datetime) -> bool:
    """Verifica se ativo ainda tem cota de sinais nesta hora."""
    try:
        _prune_symbol_signal_history(symbol, now)
        current_count = len(monitor_symbol_signal_history.get(symbol, []))
        return current_count < MONITOR_MAX_SIGNALS_PER_SYMBOL_HOUR
    except Exception:
        return True  # Se erro, deixa passar


def _register_symbol_signal_sent(symbol: str, now: datetime):
    """Registra envio de sinal para ativo, incrementando contador."""
    try:
        _prune_symbol_signal_history(symbol, now)
...
async def analyze(update: Update, context: ContextTypes.DEFAULT_TYPE, symbol=None, timeframe=None):
    ...
    try:
        ...
        # Escolhe direcao com maior pontuacao
    ...
    for symbol in symbols_to_check:
        try:
            ...
            for timeframe in timeframes_to_check:
                ...
                # Gate de qualidade com tolerância e compensação
                edge_tolerance = ATLAS_EDGE_TOLERANCE
                prob_compensation = ATLAS_PROB_COMPENSATION
                edge_compensation_factor = ATLAS_EDGE_COMPENSATION_FACTOR
                
                quality_ok = (
                    probability >= min_probability
                    and edge >= (min_edge - edge_tolerance)
                    and consensus >= min_consensus
                )
                
                compensated_quality_ok = (
                    probability >= (min_probability + prob_compensation)
                    and consensus >= (min_consensus + 1)
                    and edge >= (min_edge * edge_compensation_factor)
                )
                
                if not (quality_ok or compensated_quality_ok):
                    block_reason = (
                        f"qualidade: prob {probability:.2f}%/{min_probability:.2f}% | "
                        f"edge {edge:.4f}/{min_edge:.4f} | "
                        f"consenso {consensus}/{min_consensus}"
                    )
                    logger.info(
                        f"[POST-ATLAS-DEBUG] {symbol} {timeframe} REJEITADO: {block_reason}"
                    )
                    _register_monitor_block(symbol, timeframe, signal, block_reason, rank=probability)
                    continue
                ...
                if weekend_mode:
                    ...
                    # Setup score adaptativo por timeframe e qualidade de sinal
                    base_min_setup_score = ATLAS_MIN_SETUP_SCORE
                    ...
                    # Se prob/consenso muito fortes, relaxar score mínimo
...
🕐 {datetime.now().strftime('%H:%M:%S')}
    ...
    if alert_to_send:
        # Verificação anti-spam por ativo
        symbol = alert_to_send['symbol']
        now_dt = datetime.now()
        if not _can_send_symbol_signal(symbol, now_dt):
            remaining = MONITOR_MAX_SIGNALS_PER_SYMBOL_HOUR - len(monitor_symbol_signal_history.get(symbol, []))
            logger.info(
                f"[ANTI-SPAM] {symbol} atingiu limite de sinais/hora ({MONITOR_MAX_SIGNALS_PER_SYMBOL_HOUR}). "
                f"Próximo permitido em ~60min. (Candidato descartado)"
            )
            _register_monitor_block(
                symbol, 
                alert_to_send['timeframe'], 
                alert_to_send['signal'],
                "limite de sinais por ativo/hora",
                rank=alert_to_send.get('rank', 0),
            )
            alert_to_send = None
